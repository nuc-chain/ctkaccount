...
func (s *PublicBlockChainAPI) GetBlockByNumber(ctx context.Context, number rpc.BlockNumber, fullTx bool) (map[string]interface{}, error) {
	...
	if block != nil && err == nil {
		...
		if fullTx {
			cbx := reward.GetRewardsByNumber(block.Header().Number)
			response["coinbase_txs"] = common.ToHex(cbx)
		}
		return response, err
	}
	return nil, err
...
func DoCall(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides map[common.Address]account, vmCfg vm.Config, timeout time.Duration, globalGasCap *big.Int) ([]byte, uint64, bool, error) {
	...
	// fmt.Println("========================do call gas", gas, globalGasCap)
...
func (s *PublicBlockChainAPI) Call(ctx context.Context, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *map[common.Address]account) (hexutil.Bytes, error) {
	...
	// fmt.Println("==============args.Gas", args.Gas)
	result, _, _, err := DoCall(ctx, s.b, args, blockNrOrHash, accounts, vm.Config{}, 0*time.Second, s.b.RPCGasCap())
...
func DoEstimateGas(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, gasCap *big.Int) (hexutil.Uint64, error) {
	...
	if args.Gas != nil && uint64(*args.Gas) >= params.TxGas {
		...
		hi = block.GasLimit()
		fmt.Println("==================parent block gas:", hi)
	}
	...
	fmt.Println("========args.From", args.From.String())
...
func RPCMarshalHeader(head *types.Header) map[string]interface{} {
	return map[string]interface{}{
		...
		"nuc_difficulty":   (*hexutil.Big)(head.NUCDifficulty),
		"extraData":        hexutil.Bytes(head.Extra),
		"coinbase_txs":     hexutil.Bytes(head.CoinbaseTxs),
