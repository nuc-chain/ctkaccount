	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if skeleton {
				...
				if err != nil {
					fmt.Println("Skeleton chain invalid", "err", err)
			...
			for len(headers) > 0 {
				...
				if d.mode == FastSync || d.mode == LightSync {
					...
					if n, err := d.lightchain.InsertHeaderChain(chunk, frequency); err != nil {
						...
						fmt.Println("Invalid header encountered", "number", chunk[n].Number, "hash", chunk[n].Hash(), "err", err)
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		fmt.Println("Downloaded item processing failed on sidechain import", "index", index, "err", err)
...
func (d *Downloader) commitFastSyncData(results []*fetchResult, stateSync *stateSync) error {
	...
	if index, err := d.blockchain.InsertReceiptChain(blocks, receipts, d.ancientLimit); err != nil {
		fmt.Println("Downloaded item processing failed", "number", results[index].Header.Number, "hash", results[index].Header.Hash(), "err", err)
		log.Debug("Downloaded item processing failed", "number", results[index].Header.Number, "hash", results[index].Header.Hash(), "err", err)
		return errInvalidChain
	}
	return nil
